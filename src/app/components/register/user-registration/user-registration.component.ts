...
export class UserRegistrationComponent implements OnInit {
  ...
  constructor(private dataService: DataService, private clientService: ClientService, private formbuilder: FormBuilder, private loginService: LoginService, private countryService: CountryStateCityService, private router: Router) {
  ...
  selectEntity() {
    ...
    if (entity[0].ref1 == null) {
      ...
      this.entityName = entity[0].cmsCompanyName;
      // this.show = true;
      //this.errorMessage = "Product Admin already exists for "+this.entityName 
      this.success = "Product Admin already exists for " + this.entityName;
      $("#error").show('fast');
    }
  }
  ...
  save() {
    ...
    this.loginService.save(data)
      .subscribe((response) => {
        ...
        this.isEntity = true;
        this.success = "User id is created : " + this.userid;
        $('#success').show('fast');
      },
        (error) => {
